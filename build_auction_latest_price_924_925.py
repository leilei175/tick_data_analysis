#!/usr/bin/env python3
"""提取所有股票 09:24:00~09:25:00 区间每个 tick 时点的最新价格。

输出为单个 parquet 文件，采用长表格式，每行对应:
- 一个交易日
- 一只股票
- 09:24:00~09:25:00 区间内的一个 tick 快照

“最新价格”定义:
1. 优先使用 lastPrice（若 > 0）
2. 否则使用 (ask1 + bid1) / 2
3. 若 mid 不可用，则退化为 ask1
4. ask1 也不可用时退化为 bid1
"""
...
class Config:
    ...
    output_file: Path = Path("./factor/high_frequency/auction_latest_price_924_925/auction_latest_price_924_925_all.parquet")
    summary_file: Path = Path("./factor/high_frequency/auction_latest_price_924_925/auction_latest_price_924_925_summary.json")
...
def _extract_one_file(file_path: str, trade_date_str: str) -> Optional[pd.DataFrame]:
    ...
    try:
        df = pd.read_parquet(fp, columns=["time", "lastPrice", "lastClose", "askPrice", "bidPrice"])
    except Exception:
        return None
...
def extract_day_rows(day_dir: Path, trade_date: date, max_workers: int) -> pd.DataFrame:
    ...
    if max_workers <= 1:
        ...
        try:
            ...
            print(
                f"警告: {trade_date} 并行处理失败，自动降级为单进程重试。"
            )
            rows = []
            for file_path, trade_date_str in tasks:
                out = _extract_one_file(file_path, trade_date_str)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="提取 09:24~09:25 每个 tick 时点的最新价格")
    ...
    parser.add_argument(
        "--output-file",
        default="./factor/high_frequency/auction_latest_price_924_925/auction_latest_price_924_925_all.parquet",
        help="输出 parquet 文件",
    )
    parser.add_argument(
        "--summary-file",
        default="./factor/high_frequency/auction_latest_price_924_925/auction_latest_price_924_925_summary.json",
        help="输出 summary 文件",
    )
